...
module.exports.handleSettings = async (adapter, data) => {
	...
	await adapter.setStateChanged('has_alert', {val: false, ack: true});
	await adapter.setStateChanged('settings.language', {val: data.language, ack: true});
	await adapter.setStateChanged('settings.harvest_measurements_interval', {val: data.harvest_measurements_interval, ack: true});
	await adapter.setStateChanged('settings.harvest_status_interval', {val: data.harvest_status_interval, ack: true});
	await adapter.setStateChanged('settings.plant_id', {val: data.plant_id, ack: true});
	await adapter.setStateChanged('settings.username', {val: data.username, ack: true});
	await adapter.setStateChanged('settings.system_configured', {val: data.system_configured, ack: true});
	await adapter.setStateChanged('settings.export.type', {val: data.export_type, ack: true});
	await adapter.setStateChanged('settings.export.interval', {val: data.export_interval, ack: true});
	await adapter.setStateChanged('settings.export.host', {val: data.export_host, ack: true});
	await adapter.setStateChanged('settings.export.url', {val: data.export_url, ack: true});
	await adapter.setStateChanged('settings.export.moidom_id', {val: data.export_moidom_id, ack: true});
};
...
module.exports.handleTimeSettings = async (adapter, data) => {
	...
	await adapter.setStateChanged('has_alert', {val: false, ack: true});
	await adapter.setStateChanged('settings.location', {val: data.location, ack: true});
	await adapter.setStateChanged('settings.timezone', {val: data.timezone, ack: true});
};
...
module.exports.handleNetworkSettings = async (adapter, data) => {
	...
	await adapter.setStateChanged('settings.network.lan_mode', {val: data.lan_mode, ack: true});
	await adapter.setStateChanged('settings.network.lan_ip', {val: data.lan_ip, ack: true});
	await adapter.setStateChanged('settings.network.lan_mask', {val: data.lan_mask, ack: true});
	await adapter.setStateChanged('settings.network.lan_gateway', {val: data.lan_gateway, ack: true});
	await adapter.setStateChanged('settings.network.lan_dns', {val: data.lan_dns, ack: true});
	await adapter.setStateChanged('settings.network.wlan_mode', {val: data.wlan_mode, ack: true});
	await adapter.setStateChanged('settings.network.wlan_auth', {val: data.wlan_auth, ack: true});
	await adapter.setStateChanged('settings.network.wlan_ssid', {val: data.wlan_ssid, ack: true});
	await adapter.setStateChanged('settings.network.wlan_ip', {val: data.wlan_ip, ack: true});
	await adapter.setStateChanged('settings.network.wlan_mask', {val: data.wlan_mask, ack: true});
	await adapter.setStateChanged('settings.network.wlan_gateway', {val: data.wlan_gateway, ack: true});
};
...
module.exports.handleCloudSettings = async (adapter, data) => {
	...
	await adapter.setStateChanged('settings.cloud.url', {val: data.cloud_url, ack: true});
	await adapter.setStateChanged('settings.cloud.username', {val: data.cloud_username, ack: true});
	await adapter.setStateChanged('settings.cloud.password', {val: data.cloud_password, ack: true});
};
